...
def interpolate_frame_gpu(gpu_frame1, gpu_flow, alpha):
    h, w = gpu_frame1.size()
    
    # Generate remap coordinates on the GPU
    coords_x, coords_y = np.meshgrid(np.arange(h), np.arange(w))
    map_x = (coords_x + gpu_flow[..., 0] * alpha).astype(np.float32)
    map_y = (coords_y + gpu_flow[..., 1] * alpha).astype(np.float32)
    ...
    gpu_interpolated_frame = cv2.cuda.remap(gpu_frame1, gpu_map_x, gpu_map_y, interpolation=cv2.INTER_CUBIC)
...
while True:
    ...
    for alpha in alphas:
        ...
        gpu_interpolated_frame = interpolate_frame_gpu(gpu_frame1, gpu_flow.download(), alpha)
        ...
        flow_error = calculate_flow_consistency(gpu_flow.download(), backward_flow)
